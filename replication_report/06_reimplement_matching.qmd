...
subclasses_year <- matched_treatment %>%
  select(subclass, status_year) %>%
  unique() %>%
  collect()


test <- matched_control %>%
  group_by(subclass) %>%
...
deforestation_calc <- function(df, year, first_year = 2001, last_year = 2018) {
  C <- sum(df$cover, na.rm = TRUE)
  CL <- sum(df$cover * df$loss)
  CL_b <- NA
  CL_a <- NA
  Y_b <- NA
  Y_A <- NA
  if (year %in% 2002:(last_year - 1)) {
    df_b <- filter(df, lossyear + 2000 %in% 2001:(year - 1)) 
    df_a <- filter(df, lossyear + 2000 %in% (year + 1):last_year)
    CL_b <- sum(df_b$cover * df_b$loss)
    CL_a <- sum(df_a$cover * df_a$loss)
    Y_b = year - first_year
    Y_a = last_year - year
  }
  out <- c(loss = - (((C - CL)/C)^(1/18) - 1),
...
test_country_num <- 190 # Rwanda

test_trt <- matched_treatment %>%
  filter(country_num == test_country_num) 

test_crtl <- matched_treatment %>%
  filter(country_num == test_country_num)

test_out <- test_trt %>%
  mutate(control_vars = deforestation_calc(
    df = filter(test_crtl, .$subclass_ctrl == subclass_trt),
    year = status_year))
