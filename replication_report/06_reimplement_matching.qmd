...
if (!file.exists("revamp/countries/ne_10m_admin_0_countries.shp")) {
  ...
  countries <- st_read("revamp/countries/ne_10m_admin_0_countries.shp")
}
...
for (i in 1:3) {
  ...
  ee_region <- aois_ee$filter("GID_0 == 'URY'") #$geometry()
  test_region <- ee_as_sf(ee_region, dsn = "temp/uruguay.geojson", 
                          via = "drive", lazy = TRUE) 
  uru <- ee_utils_future_value(test_region)
  
  tmap_mode("view")
  uru <- mutate(uru, area = st_area(uru))
  tm_shape(st_make_valid(countries)) + 
    tm_borders()
  
  aois_tbl$task[i] <- ee_as_raster(
    combined, via = "drive", container = "rgee_test1", lazy = TRUE,
    region = ee_region, crs = "EPSG:4326", maxPixels = 1e13,
