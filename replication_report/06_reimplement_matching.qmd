...
# Write data
if (Sys.getenv("USER") == "onyxia") {
  aws.s3::s3write_using(FUN = arrow::write_parquet,
                        x = matched_data,
                        object = "Replication_wolf/matched_data.parquet",
                        bucket = "fbedecarrats",
                        opts = list("region" = ""))
} else {
...
matched_treatment <- open_dataset("revamp/consolidated/matched_data/is_pa=1") %>%
  select(country_num, rownum_trt = rownum, wdpa_id, status_year, subclass)
matched_control <- open_dataset("revamp/consolidated/matched_data/is_pa=0") %>%
  select(country_num, rownum_ctrl = rownum, subclass)
...
deforestation_calc <- function(df, year, name_prefix = "") {
