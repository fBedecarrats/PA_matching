...
library(arrow)
my_schema <- schema(field(name = "cover", type = int8()),
                    field(name = "loss", type = int8()),
                    field(name = "lossyear", type = int8()),
                    field(name = "elev", type = int16()),
                    field(name = "slope", type = int8()),
                    field(name = "travel_time", type = int32()),
                    field(name = "pop_dens", type = int32()),
                    field(name = "country_num", type = int16()),
                    field(name = "biome_num", type = int16()),
                    field(name = "is_pa", type = int8()),
                    field(name = "is_pa_buffer", type = int8()),
                    field(name = "wdpa_id", type = int32()),
                    field(name = "status_year", type = int16()))
...
for (k in 1:length(my_countries)) {
  ...
  for (my_tif in my_tifs) {
    ...
    my_df <- as.data.frame(my_rast) %>%
      filter(country_num > 0 & biome_num > 0)
...
# Now we'll store all this into a correctly partitionned parquet dataset file
dir.create("revamp/tabular_struct")
consolidated <- open_dataset("revamp/tabular_country", my_schema)
write_dataset(consolidated, path = "revamp/tabular_struct",
              partitioning = c("biome_num", "country_num"))

```

Ultimately, we have 201 national territories for which we download data.