...
```{r}
dir.create("revamp/countries/coarse")
# simpler geometries for countries
countries_coarse <- world(resolution=5, level=0, 
                          path = "revamp/countries/coarse")
countries_coarse_buff <- countries_coarse %>%
  st_as_sf() %>%
  st_make_valid() %>%
  st_buffer(dist = 1) %>%
  st_make_valid()
...
i = 157
for (i in 157:157) {
  country_name <- countries_coarse_buff$NAME_0[i]
  GID <- countries_coarse_buff$GID_0[i]
  print(country_name)
  
  aoi_region <- aois_ee$filter(paste0("GID_0 == '", GID,"'"))
  aoi_extract <- prepare_gee_data(aoi_region)
  gee_tasks$task[[i]] <- ee_as_raster(
    aoi_extract, via = "drive", container = "rgee_batch", lazy = TRUE,
    dsn = paste0("revamp/rgee/", GID, "_4.tif"),
