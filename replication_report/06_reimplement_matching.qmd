...
if (Sys.getenv("USER") == "onyxia") {
  all_df <- aws.s3::s3read_using(FUN = arrow::read_parquet,
                                 object = "Replication_wolf/gee_data.parquet",
                                 bucket = "fbedecarrats",
                                 opts = list("region" = ""))
} else {
...
# clean memory
rm(matched_data, matched_all, all_df)
```
...
country_nums <- matched_treatment %>%
  select(country_num) %>%
  unique() %>%
  collect() %>%
  pluck("country_num")

join_treat_control <- function(x) {
  inner_join(filter(matched_treatment, country_num == x),
             filter(matched_control, country_num == x),
             by = join_by(subclass),
              suffix = c("treatment_", "control_")) %>%
    group_by(treatment_country_num) %>%
    write_dataset("revamp/consolidated/joined_data")
}


test <- inner_join(filter(matched_treatment, country_num == 2),
             filter(matched_control, country_num == 2),
             by = join_by(subclass)) 

map(country_nums, join_treat_control)

map(country_nums, print)


matched_treatment <- read_parquet("revamp/consolidated/matched_treatment.parquet",
                                  as_data_frame = FALSE)
matched_control <- read_parquet("revamp/consolidated/matched_control.parquet",
                                  as_data_frame = FALSE)
tic()
matched_treatment %>%
  inner_join(matched_control, by = join_by(is_pa), 
             suffix = c("treatment_", "control_")) %>%
  write_parquet("revamp/consolidated/matched_joined.parquet")
toc()
...
tic()
write_parquet(matched_data, matched_parquet_loc)
toc() 

matched_data <- read_parquet(matched_parquet_loc, as_data_frame = FALSE)




matched_parquet_loc <- "revamp/consolidated/matched_data.parquet"
tic()
matched_treatment <- filter(matched_data, is_pa == 1)
toc()
tic()
matched_controls <- filter(matched_data, is_pa == 0)
toc()
