...
prepare_gee_data <- function(ee_region) {
  ...
  aoi_cover2 <- aoi_gfc2$select("treecover2000")$
    reduceResolution(reducer = ee$Reducer$mean(), bestEffort = TRUE)$toInt()
  aoi_loss2 <- aoi_gfc2$select("loss")$
    reduceResolution(reducer = ee$Reducer$mean(), bestEffort = TRUE)$toInt()
  aoi_lossyear2 <- aoi_gfc2$select("lossyear")
  # From Rodrigo Principe classes
  years <- ee$List$sequence(1, 21)
  annual_loss <- function(year = 1) {
    year_i <- ee$Number(year)
    mask <- aoi_lossyear2$eq(year_i)
    return(aoi_lossyear2$updateMask(mask)$set("loss in year", year_i))
  }
  loss_by_year <- years$map(annual_loss)


  # From innovar
  years <- c(1:21)
  names(years) <- c(2001:2021)
  # This creates an image with 1 band per year
  aoi_lossyear2 <- aoi_gfc2$select("lossyear")$eq(years)

  
  library(innovar)
  
  test <- aois[18,]
  test2 <- get_def(from = "2001-01-01", to = "2004-12-31", region = ee_region, 
                   scale = 1000)
...
aois_ee <- ee$FeatureCollection("projects/ee-fbedecarrats/assets/aois")
for (i in 181:length(my_countries)) {
  GID <- my_countries[i]
