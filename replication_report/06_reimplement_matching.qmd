...
prepare_gee_data <- function(ee_region) {
  ...
  aoi_image <- aoi_image$addBands(
    wdpa_reg$reduceToImage(properties = list("PA"),
                           reducer = ee$Reducer$first())$toInt())
  aoi_image <- aoi_image$addBands(
    wdpa_reg$reduceToImage(properties = list("STATUS_YR"),
                           reducer = ee$Reducer$first())$toInt())
  # Add a buffer around the protected areas
  is_pa_buffer <- wdpa_reg$geometry()$buffer(10000)
  is_pa_buffer <- ee$FeatureCollection(is_pa_buffer)$
    map(function(pa) { return(pa$set("PA_buffer", 1)) })
  aoi_image <- aoi_image$addBands(
    is_pa_buffer$reduceToImage(properties = list("PA_buffer"),
                               reducer = ee$Reducer$first())$toInt())
  names(aoi_image) <- c("cover", "loss", "lossyear", "elev", "slope",
                        "travel_time", "pop_dens", "country_num", "biome_num",
                        "wdpa_id", "is_pa", "status_year", "is_pa_buffer")
  # ADD CLIP /!\
...
gee_tasks <- tibble(country = my_countries)
for (i in 18:length(my_countries)) {
  print(continents[i])
  ...
  gee_tasks$task[[i]] <- ee_as_raster(
    ...
    dsn = paste0("revamp/rgee/continent_", i, ".tif"),
    skipEmptyTiles = TRUE, scale = 1000,
    region = aoi_region$geometry())
}
# Failing at:
# "Process Tropical & Subtropical Moist Broadleaf Forests for New Zealand 18/364"
