...
write_rds(gee_out_list, "revamp/gee_out_list.Rdata")
```
...
# my_countries <- "PER"
gee_out_list <- read_rds("revamp/gee_out_list.Rdata")
for (j in 5:length(gee_out_list)) {
  ee_utils_future_value(out_list[[j]])
}
...
rm(list = c("matched_data", "matched_all"))
```
...
subset_trt <- matched_treatment %>%
  filter(country_num == 175 & status_year %in% 2001:2018)

subset_ctrl <- matched_control %>%
  select(-status_year) %>%
  filter(country_num == 175)
...
defor_computed <- defor_to_compute %>%
  ...
  select(subclass, status_year, loss, loss_before, loss_after)
