...
my_inputs <- my_files %>%
  filter(str_detect(Key, "/data_input/")) %>%
  filter(str_detect(Key, "/gee/", negate = TRUE)) %>%
  filter(str_detect(Key, "/range_maps/", negate = TRUE)) %>%
  pluck("Key")

# generate pathes
my_inputs_dest <- str_remove(my_inputs, "Replication_wolf/")
# copy locally input files
map2(my_inputs, my_inputs_dest, save_from_s3)
# copy locally rasters processed before 004 - join_rasters.R
...
save_from_s3(from = "Replication_wolf/aau3445-data-s3.tif", 
             to = "data_input/threatened.tif")
# Avoid re-downloading existing data
replace_all(pattern = "system\\(paste0",
            replacement = "# system\\(paste0",
            file = join_script)



# Replace lossyear by lossyear new
file.remove("data_input/lossyear.tif")
file.rename("data_input/lossyear_manual_reduce.tif", "data_input/lossyear.tif")
