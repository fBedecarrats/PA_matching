...
replace_all(pattern = 'read_sf\\("data_input/PAs/WDPA_Jan2020-shapefile-polygons.shp"\\)',
            replacement = 'wdpar::wdpa_read\\("data_input/WDPA_Jan2020_Public.zip"\\) %>% dplyr::filter\\(st_geometry_type(\\.\\) != "MULTIPOINT"\\)',
            file = join_script)
...
replace_all(pattern = 'read_sf\\("data_input/PAs/WDPA_Jan2020-shapefile-polygons.shp"\\)',
            replacement = 'wdpar::wdpa_read\\("data_input/PAs/WDPA_Jan2020_Public.zip"\\)',
            file = join_results_script)
...
```{r}
PAs <- readRDS("data_processed/PA_df.RDS")
