...
data <- read_parquet("PA_matching_asis/data_processed/all_data_wolf.parquet")
...
# tic()
matched_data %>%
  group_by(treatment, countries) %>%
  write_dataset("PA_matching_asis/consolidated/matched_data")

country_list <- group_keys(group_by(matched_data, countries)) %>%
  pluck("countries")
...
# We loop over country number to avoid saturating memory
tic()
df <- data.frame()
for (i in country_list) {
  print(i)
  temp <- calc_defor_mode(
    first_year = 2001,
    last_year = 2018,
    matched_treatment = open_dataset(
      paste0("PA_matching_asis/consolidated/matched_data/",
             "treatment=1/countries=", i)),
    matched_control  = open_dataset(
      paste0("PA_matching_asis/consolidated/matched_data/",
             "treatment=0/countries=", i)))
...
rm(list = c("data", "matched_all", "matched_data", "temp"))

stats <- df %>%
  summarise(loss_trt = mean(trt_loss, na.rm = TRUE),
            loss_ctrl = mean(ctrl_loss, na.rm = TRUE),
            loss_trt_before = mean(trt_loss_before, na.rm = TRUE),
            loss_ctrl_before = mean(ctrl_loss_before, na.rm = TRUE),
            loss_trt_after = mean(trt_loss_after, na.rm = TRUE),
            loss_crtl_after = mean(ctrl_loss_after, na.rm = TRUE))


hist(df$trt_loss, breaks = 100, xlim = 0.2)
hist(df$ctrl_loss, breaks = 100)
